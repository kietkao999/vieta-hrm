...
const FILE_TYPE_CHECKLIST = [
  { id: 'HOA_DON_GTGT', label: 'Hóa đơn Giá trị gia tăng (GTGT)' },
  { id: 'BAO_GIA_HOP_DONG', label: 'Báo giá & Hợp đồng kinh tế' },
  { id: 'BIEN_BAN_NGHIEM_THU', label: 'Biên bản nghiệm thu / Bàn giao thực tế' },
  { id: 'ANH_THUC_TE', label: 'Hình ảnh thi công / Nghiệm thu hiện trường' },
...
const PrintWorkflowModal = ({ request, type = 'PURCHASE', isOpen, onClose }) => {
  ...
  const formTitle = isPurchase ? 'GIẤY ĐỀ NGHỊ MUA DỊCH VỤ' : 'GIẤY ĐỀ NGHỊ THANH TOÁN';
    ...
    <div className="fixed inset-0 z-70 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto print:p-0 print:bg-white print:static print:inset-auto">
      <div className="relative w-full max-w-4xl bg-white rounded-xl shadow-2xl overflow-hidden print:shadow-none print:w-full print:max-w-none print:rounded-none">
        ...
        >
          ...
          {/* NỘI DUNG PHẦN 3: Đề nghị thanh toán */}
          {!isPurchase && (
          ...
          <div className="mb-6 p-3 bg-slate-50 border border-slate-300 rounded-md text-xs">
            ...
            <div className="grid grid-cols-2 gap-2">
              {FILE_TYPE_CHECKLIST.map((item) => {
                const isChecked = attachedTypes.includes(item.id);
                return (
