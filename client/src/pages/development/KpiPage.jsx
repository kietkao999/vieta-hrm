...
const KpiPage = () => {
  ...
  const filteredList = useMemo(() => {
    const selectedDeptObj = departments.find(d => String(d.id) === String(selectedDept) || d.name === selectedDept);
    return kpiList.filter(item => {
      ...
      const matchDept = !selectedDept || 
        String(item.department_id) === String(selectedDept) ||
        item.department_name === selectedDept ||
        (selectedDeptObj && item.department_name === selectedDeptObj.name) ||
        (selectedDeptObj && String(item.department_id) === String(selectedDeptObj.id));

      return matchSearch && matchDept;
    });
  }, [kpiList, searchTerm, selectedDept, departments]);
  ...
  const summaryMetrics = useMemo(() => {
    ...
    return {
      totalEmployees: filteredList.length,
      totalRespNet,
      totalPerfNet,
      totalKpiSum
    };
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative flex-1 md:w-60">
            <input
              ...
              className="w-full border border-slate-200 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-700 outline-none focus:border-brand-500 placeholder-slate-400"
            />
          ...
          <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-2.5 py-1.5 rounded-lg whitespace-nowrap">
            Hiển thị: <b className="text-brand-700">{filteredList.length}</b>/{kpiList.length} NV
          </span>
        </div>
      </div>
                        ...
                        {canEdit ? (
                          <div className="flex flex-col items-center justify-center space-y-1">
                            <div className="flex items-center justify-center space-x-1">
                              <input
                                ...
                                max="100"
                                step="0.01"
                                value={item.raw_rate_input !== undefined ? item.raw_rate_input : (item.responsibility_rate !== undefined ? (Math.round(item.responsibility_rate * 10000) / 100) : 100)}
                          ...
                          <div className="flex flex-col items-center">
                            <span className="text-xs font-bold text-slate-800">
                              {item.responsibility_rate !== undefined ? (Math.round(item.responsibility_rate * 10000) / 100) : 100}%
                            </span>
