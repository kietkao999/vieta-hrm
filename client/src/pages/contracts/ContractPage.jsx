...
import { FileText, AlertTriangle, Plus, Search, Filter, Edit2, Trash2 } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalType === 'create') {
        ...
        await api.put(`/contracts/${selectedContract.id}`, formData);
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {contracts.length === 0 ? (
                ...
                const daysLeft = getRemainingDays(c.end_date);
                const isWarning = daysLeft !== null && daysLeft > 0 && daysLeft <= 30;
                const isExpired = daysLeft !== null && daysLeft <= 0;

                return (
                  <tr key={c.id} className="hover:bg-slate-50">
                    <td className="px-4 py-4 font-mono font-semibold text-brand-700">{c.contract_number}</td>
                    ...
                    <td className="px-4 py-4">
                      ...
                      {isExpired && c.status === 'Có hiệu lực' && <span className="flex items-center text-red-600 text-xs font-bold"><AlertTriangle size={14} className="mr-1"/> Đã quá hạn</span>}
                      {(!isWarning && !isExpired && c.status === 'Có hiệu lực' && daysLeft !== null) && <span className="text-emerald-600 text-xs font-medium">Còn {daysLeft} ngày</span>}
                    </td>
                    <td className="px-4 py-4">
                       <span className={`px-2 py-1 rounded text-xs font-bold ${
                        c.status === 'Có hiệu lực' ? 'bg-emerald-100 text-emerald-700' :
                        c.status === 'Hết hạn' ? 'bg-amber-100 text-amber-700' :
                    ...
                    {(user?.roleName === 'ADMIN' || user?.roleName === 'HR') && (
                      <td className="px-4 py-4 text-right space-x-2">
                        <button onClick={() => handleOpenEdit(c)} className="text-slate-500 hover:text-brand-600"><Edit2 size={16}/></button>
                        {user?.roleName === 'ADMIN' && (
