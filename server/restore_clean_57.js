...
export const OFFICIAL_57_EMPLOYEES = [
...
async function restore() {
  console.log('=== BẮT ĐẦU KHÔI PHỤC DANH SÁCH 57 NHÂN SỰ CHÍNH THỨC ===');

  const validCodes = OFFICIAL_57_EMPLOYEES.map(line => line.split('|')[0].trim());

  // 1. Xóa các nhân sự không thuộc 57 nhân sự chính thức
  const placeholders = validCodes.map(() => '?').join(',');
  const invalidEmps = await all(`SELECT id, code, fullname FROM employees WHERE code NOT IN (${placeholders})`, validCodes);
  console.log(`Tìm thấy ${invalidEmps.length} nhân sự tạo thừa cần dọn dẹp.`);

  if (invalidEmps.length > 0) {
    const invalidIds = invalidEmps.map(e => e.id);
    const idPlaceholders = invalidIds.map(() => '?').join(',');
    await run(`DELETE FROM employee_monthly_kpis WHERE employee_id IN (${idPlaceholders})`, invalidIds);
    await run(`DELETE FROM payrolls WHERE employee_id IN (${idPlaceholders})`, invalidIds);
    await run(`DELETE FROM employees WHERE id IN (${idPlaceholders})`, invalidIds);
    console.log(`✓ Đã dọn dẹp sạch ${invalidEmps.length} nhân sự thừa!`);
  }

  const remaining = await get('SELECT COUNT(*) as count FROM employees');
  console.log(`Số nhân sự còn lại trong hệ thống: ${remaining.count} (Chuẩn 57 nhân sự)`);
